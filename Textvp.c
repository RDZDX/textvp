    ...
    for (i = 0; i < 20; i++)  // 20 prasisukimu - 20 tekstiniu ekrano eiluciu, kadangi uodega
    {
        ...
        while (*ptr != '\0') {
            if (trigeris == 0) {
                ...
                sprintf(vns_simbl, "%c", *ptr); // ar rezultate nesigauna simbolis + '/0' ??????????????????????? 

                vm_chset_convert(VM_CHSET_UTF8 , VM_CHSET_UCS2, vns_simbl, knv_vns_simblX, (strlen(vns_simbl) + 1) * 2);
                
                vns_simbl_plot = vm_graphic_get_character_width((VMWCHAR)knv_vns_simblX); // Turim VMCHAR, reikia VMWCHAR
                //vns_simbl_plot = vm_graphic_get_character_width(knv_vns_simblX); // Turim VMCHAR, reikia VMWCHAR
