...
void handle_keyevt(VMINT event, VMINT keycode) {
 ...
 if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_NUM6) {
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_NUM3) {
          ...
          if (color1 == VM_COLOR_WHITE && trigeris6 == VM_FALSE) {
          //if (color1 == VM_COLOR_BLACK && trigeris6 == VM_FALSE) {
...
void vertical_scrolling_ucs2_text(VMWSTR ucs2_string) {
    ...
    vm_graphic_textout_by_baseline(buffer, 0, filledDsplByLines + abc, ucs2_string, vm_wstrlen(ucs2_string), color2, 12);
...
void mre_draw_black_rectangle(void) {
    ...
    vm_graphic_fill_rect(buffer, 0, 0, vm_graphic_get_screen_width(), vm_graphic_get_screen_height(), color1, color1);
    vm_graphic_flush_layer(layer_hdl, 1);
}
/*****************************************************************************
 * FUNCTION
