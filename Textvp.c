...
//    VMWCHAR value21[MRE_STR_SIZE_MAX + 23] = {0};
//    VMWCHAR konv_stringas[MRE_STR_SIZE_MAX + 23] = {0};
//    VMWCHAR knv_vns_simbl[MRE_STR_SIZE_MAX] = {0};
    ...
    for (i = 0; i < 20; i++)  // 20 prasisukimu - 20 tekstiniu ekrano eiluciu, kadangi uodega
    {
        ...
        vm_chset_convert(VM_CHSET_UTF8 , VM_CHSET_UCS2, value10, value21, 286);
        ...
        while (*ptr != '\0') {
            if (trigeris == 0) {
                ...
                vm_chset_convert(VM_CHSET_UTF8 , VM_CHSET_UCS2, vns_simbl, knv_vns_simbl, 286);
                //vns_simbl_plot = vm_graphic_get_string_width(knv_vns_simbl);
                vns_simbl_plot = vm_graphic_get_character_width(knv_vns_simbl);

            }

            if ((strng_plot + vns_simbl_plot) > 243 && trigeris == 0) {

                vm_chset_convert(VM_CHSET_UTF8 , VM_CHSET_UCS2, nauj_strng, konv_stringas, 286);
...
void vertical_scrolling_ucs2_text(VMWSTR ucs2_string) {
    ...
    int font_height = vm_graphic_get_character_height();
    max_height = vm_graphic_get_screen_height();


	//for (i = 0; i < wstrlen (ucs2_string); i++)
	//{
	//	int width, height;
	//	vm_graphic_measure_character (ucs2_string[i], &width, &height);
...
static void draw_result_ucs2_string(int display_string_x_pos, int display_string_y_pos, VMWSTR ucs2_string) {
    ...
    //                   display_string_x_pos, display_string_y_pos, ucs2_string,
    //                   wstrlen(ucs2_string), VM_COLOR_WHITE);
