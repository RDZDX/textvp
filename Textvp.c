...
void handle_keyevt(VMINT event, VMINT keycode) {
 ...
 if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_RIGHT_SOFTKEY) {
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_UP) {
       ...
       if (failoGalas == 1 && lastPressedKey == 4) {
           mre_draw_black_rectangle();
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_DOWN) {
       ...
       if (failoGalas == 1 && lastPressedKey == 2) {  //Pagal mane neveikianti salyga
       ...
       if (failoGalas == 1 && lastPressedKey == 3) {
       }

       if (failoGalas == 1 && lastPressedKey == 4) {  //Pagal mane neveikianti salyga
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_NUM1) {
       ...
       if (lastPressedKey == 4 && failoGalas == 1){
       ...
       else if (lastPressedKey == 3 && failoGalas == 1){
...
VMINT mre_read_file_display(VMWSTR path) {
    ...
    strcpy(value10, "");     
    //myFilePosition = 0;
    //vm_file_seek(f_read, myFilePosition, BASE_END); //?????????????????????????????????? 
    filledDsplByLines = 0;
    nauj_strng_ilg = 0;
    failoGalas = 1;
    stopCounter = 1;
