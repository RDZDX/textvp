...
VMINT mre_read_file_display(VMWSTR path) {
    ...
    plusPage = 0;
    xuodega = 0;
    int prasisk_count1 = 0;
    int prasisk_count2 = 0;
    int ilgism = 0;
    int ilgisr = 0;
    ...
    for (i = 0; i < 20; i++)  // constants: 20 text lines on 320' display
    {
        ...
        vm_file_read(f_read, value1, 60, &nread);  // constants: on 240' display on one text line 60 slimest symbols "." or 15 widest symbols "--"
        value1[nread] = '\0';
        dynamicPosition = vm_file_tell(f_read);  // Issaugome dabartine kursoriaus pozicija faile
        strcat(value10, value1);
        ilgisr = strlen(value10);
        vm_chset_convert(VM_CHSET_UTF8, VM_CHSET_UCS2, value10, value21, (ilgisr + 1) * 2);
        //vm_ascii_to_ucs2(value21, (ilgisr + 1) * 2, value10);
    ...
    strcpy(value10, "");
    strcpy(value21, "");
    //vm_wstrcpy(value21, (VMWSTR)"");
    strcpy(konv_stringas, "");
    //vm_wstrcpy(konv_stringas, (VMWSTR)"");
