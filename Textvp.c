...
void handle_keyevt(VMINT event, VMINT keycode) {
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_NUM8) {
       ...
       x10_puslapiu_adr = dynamicPosition + (simbl_kiek_pirm_lape * 4);

       if (x10_puslapiu_adr < endFlPosition - 380) {
          mre_draw_black_rectangle();
          dynamicPosition = x10_puslapiu_adr;
          fiksedPosition = x10_puslapiu_adr;
          prviousFlPosit = x10_puslapiu_adr;
...
VMINT mre_read_file_display(VMWSTR path) {
    ...
    for (i = 0; i < 20; i++)  // 20 prasisukimu - 20 tekstiniu ekrano eiluciu, kadangi uodega
    {
        ...
        value1[nread] = '\0';
        dynamicPosition = vm_file_tell(f_read);  // Issaugome dabartine kursoriaus pozicija
    ...
    if (lapu_kiekis == 0) {
       ...
       lapu_kiekis = (endFlPosition / dynamicPosition) + 1;
       //lapu_kiekis = ceil(8.33);
       //lapu_kiekis = ceil(endFlPosition / dynamicPosition);
...
void vertical_scrolling_ucs2_text(VMWSTR ucs2_string) {
    ...
    vm_graphic_textout_by_baseline(vm_graphic_get_layer_buffer(layer_hdl[0]), MRE_SCREEN_START_X, filledDsplByLines + abc, ucs2_string, wstrlen(ucs2_string), VM_COLOR_WHITE, 12);
    //vm_graphic_textout_by_baseline(vm_graphic_get_layer_buffer(layer_hdl[0]), MRE_SCREEN_START_X, filledDsplByLines + abc, ucs2_string, wstrlen(ucs2_string), VM_COLOR_BLACK, 12);
